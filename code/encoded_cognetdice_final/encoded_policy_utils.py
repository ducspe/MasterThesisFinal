...
# My libraries:
from encoded_graph_network import EncodedGNet

sim = "This simulation object should be set by the transfer_info function"
obs_vae = "This is the Variational Autoencoder object that should be set by the transfer_info function"
...
def transfer_info(simulation, observation_var_autoenc, N_hidden_nodes, results_directory, nr_nodes_list, logger):
    global sim, obs_vae, nr_hidden_nodes, results_directory_name, number_of_nodes_per_agent, my_logger
    sim = simulation
    obs_vae = observation_var_autoenc
...
def rollout(joint_policy, horizon, decpomdp_simulator, discount_factor):
    ...
    actions_taken = np.zeros(shape=(horizon, decpomdp_simulator.num_agents), dtype=int)
    observations_received = np.zeros(shape=(horizon + 1, decpomdp_simulator.num_agents, decpomdp_simulator.patch_height, decpomdp_simulator.patch_width), dtype=float)
    encoded_observations = np.zeros(shape=(horizon + 1, decpomdp_simulator.num_agents), dtype=float)
    nodes_visited = np.zeros(shape=(horizon + 1, decpomdp_simulator.num_agents), dtype=int)
    ...
    for t in range(horizon):
        ...
        for agent in range(decpomdp_simulator.num_agents):
            enc_obs_mu, encoded_obs_var = obs_vae.encode_numpy(observations_received[t+1, agent, :, :])
            encoded_observations[t+1, agent] = enc_obs_mu

        nodes_visited[t+1, :] = joint_policy.sample_next_nodes(nodes_visited[t, :], encoded_observations[t+1, :])

    return actions_taken, nodes_visited, encoded_observations, cumulative_reward
...
def inject_entropy_without_collecting_new_data(joint_policy, decpomdp_simulator, theta_ei):
    ...
    for agent in range(decpomdp_simulator.num_agents):
        for current_node in range(joint_policy.local_policies[agent].num_nodes):
            ...
            joint_policy.local_policies[agent].nodes[current_node].action_distribution = (1 - theta_ei) * joint_policy.local_policies[agent].nodes[current_node].action_distribution + theta_ei * maximum_entropy_distribution
...
class LocalPolicyNode:
    def __init__(self, node_index, num_actions, num_nodes):
        ...
        self.classifier = EncodedGNet(nr_hidden_nodes=nr_hidden_nodes, nr_output_nodes=num_nodes)
...
class JointPolicy:
    ...
    def set_simulation_deterministic_attributes(self, action_indices, current_node_indices, horizon):
        ...
        for agent in range(self.num_local_policies):
            for iterated_node in range(self.local_policies[agent].num_nodes):
                for t in range(horizon):
                    if current_node_indices[t, agent] == iterated_node:
                        if not flag_node[iterated_node][agent]:
                            self.local_policies[agent].nodes[current_node_indices[t, agent]].action_distribution = np.zeros(shape=(self.local_policies[agent].num_actions))

                        np.put(self.local_policies[agent].nodes[current_node_indices[t, agent]].action_distribution, action_indices[t, agent], 1)

                        if flag_node[iterated_node][agent]:
                            self.local_policies[agent].nodes[current_node_indices[t, agent]].action_distribution /= np.sum(self.local_policies[agent].nodes[current_node_indices[t, agent]].action_distribution)

                        flag_node[iterated_node][agent] = True
