...
def draw_trained_model_line_response(policy, model_directory_path, number_of_nodes_per_agent, sim, title_suffix=""):
    for agent_index in range(sim.num_agents):
        N_nodes = number_of_nodes_per_agent[agent_index]
        for q in range(N_nodes):
            observation_samples, next_nodes_likelihood = encoded_graph_network.show_node_input_response(joint_policy=policy, observation_interval=(-3, 3), agent=agent_index, node=q, num_divisions=50)
            ...
            plt.xlabel("Latent Z Observation")
...
def draw_trained_model_bar_response(policy, model_directory_path, number_of_nodes_per_agent, sim, title_suffix=""):
    for agent_index in range(sim.num_agents):
        ...
        for q in range(N_nodes):
            ...
            observation_samples, next_nodes_list = encoded_graph_network.show_node_input_response(joint_policy=policy, observation_interval=(-3, 3), agent=agent_index, node=q, num_divisions=10)
