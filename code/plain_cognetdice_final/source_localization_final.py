...
if __name__ == "__main__":
    ...
    ############################################################
    # Define parameters:
    sim = Simulator("simCC.txt")
    number_of_nodes_per_agent = [3 for tagger in range(sim.num_taggers)]
    number_of_actions_per_agent = [sim.num_actions(tagger) for tagger in range(sim.num_taggers)]
    ...
    best_policy, best_value = cognetdice_final.run_cognetdice(results_directory_name, my_logger, sim, number_of_nodes_per_agent, number_of_actions_per_agent, alpha, discount_factor, horizon, N_rollouts, N_iterations, N_top_rollouts, N_top_average, N_evals, N_epochs, batch_size, N_hidden_nodes)
    ...
    utils.draw_trained_model_bar_response(best_policy, f"./{results_directory_name}", number_of_nodes_per_agent, sim, title_suffix="final")
    utils.draw_trained_model_line_response(best_policy, f"./{results_directory_name}", number_of_nodes_per_agent, sim, title_suffix="final")
