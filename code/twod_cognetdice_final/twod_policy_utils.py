...
# My libraries:
from twod_graph_network import TwodGNet
...
def rollout(joint_policy, horizon, decpomdp_simulator, discount_factor):
    ...
    observations_received = np.zeros(shape=(horizon + 1, decpomdp_simulator.num_agents, sim.patch_height, sim.patch_width), dtype=float)
...
class LocalPolicyNode:
    def __init__(self, node_index, num_actions, num_nodes):
        ...
        self.classifier = TwodGNet(nr_hidden_nodes=nr_hidden_nodes, nr_output_nodes=num_nodes, image_height=sim.patch_height, image_width=sim.patch_width)
